...
namespace Vst {
	...
	template<typename T>
	void IirLpf<T>::clearBuffer() {
		for (int i = 0; i < 4; i++) {
			this->filterBuffer[i] = 0;
		}
	}

	template<typename T>
	void IirLpf<T>::setCutoffFrequency(T cutoffFrequency) {
		this->cutoffFrequency = cutoffFrequency;
		this->calcCoef();
	}

	template<typename T>
	T IirLpf<T>::getCutoffFrequency() {
		return this->cutoffFrequency;
	}

	template<typename T>
	void IirLpf<T>::calcCoef() {
	...
	template<typename T>
	void IirLpf<T>::setSampleRate(SampleRate sampleRate) {
